...
<template>
  <div class="ios-weather-icon" :style="{ width: `${size}px`, height: `${size}px` }">
    ...
    <svg v-if="iconType === '01d'" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Sun Core -->
      <circle cx="32" cy="32" r="15" fill="url(#sun-glow)" filter="url(#sun-shadow)" />
      <!-- Sun Rays -->
      <g stroke="#FFA800" stroke-width="3" stroke-linecap="round" opacity="0.85">
    ...
    <svg v-else-if="iconType === '02d'" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Sun in background -->
      <circle cx="24" cy="22" r="12" fill="url(#sun-back)" />
      <g stroke="#FFA800" stroke-width="2.5" stroke-linecap="round" opacity="0.75">
    ...
    <svg v-else-if="['09d','09n','10d','10n'].includes(iconType)" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      />
      <!-- Raindrops -->
      <rect x="22" y="46" width="3.5" height="9" rx="1.75" fill="url(#drop-grad)" transform="rotate(15 23.75 50.5)" />
      <rect x="32" y="47" width="3.5" height="9" rx="1.75" fill="url(#drop-grad)" transform="rotate(15 33.75 51.5)" />
      <rect x="42" y="46" width="3.5" height="9" rx="1.75" fill="url(#drop-grad)" transform="rotate(15 43.75 50.5)" />
    </svg>
    ...
    <svg v-else-if="['11d','11n'].includes(iconType)" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <polygon points="32,38 25,50 31,50 28,60 38,46 32,46" fill="#FACC15" />
      <path
    ...
    <svg v-else-if="['13d','13n'].includes(iconType)" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        ...
        fill="#CBD5E1"
      />
      <circle cx="23" cy="51" r="2.5" fill="#E2E8F0" />
      <circle cx="33" cy="54" r="2.5" fill="#E2E8F0" />
      <circle cx="43" cy="51" r="2.5" fill="#E2E8F0" />
    </svg>
    ...
    <svg v-else viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <circle cx="32" cy="32" r="16" fill="url(#sun-def)" />
    </svg>
  </div>
</template>
